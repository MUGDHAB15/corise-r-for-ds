...
tbl_names_popular |> 
  ...
  labs(
    ...
    subtitle = 'Emma and Noah are the most popular baby names',
    caption = 'Source: SSA',
    x = '# Births',
    y = 'Name'
  ) +
...
tbl_names_by_letter |> 
  ...
  ggplot(aes(y=pct_births, x=first_letter, color = sex)) +
  ...
  # Add labels (title, subtitle, x, y)
    labs(
      title = "% of babies per first letter and sex",
      subtitle = "xxx",
      x = 'First Letter',
      y = '% babies'
    ) +
...
plot_trends_in_letter <- function(my_letter) {
  tbl_names_by_letter |> 
    ...
    ggplot(aes(y = pct_births, x = year, color = sex)) +
    # Add a line layer
     geom_line() +
    # Add labels (title, subtitle, caption, x, y)
   labs(
      title = glue::glue("Trends in Names beginning with {my_letter}"),
      subtitle = "Trend sex of the baby",
      caption = "You can call the function with any letter",
      x = "Year",
      y = '% Births'
    ) +
...
tbl_names_by_first_and_last_letter |> 
  ...
  # Add labels (title, subtitle, x, y, fill)
   labs(
      title = "Frequency of first/last letter combinations" ,
      subtitle = "Letter Combination",
      caption = "source:ssa",
      x = "Last letter",
      y = 'First Letter'
    ) +
...
tbl_names_vowel_consonant |> 
  ...
  geom_area(fill = first_last) +
  # Facet wrap plot by `sex`
  facet_wrap(~ sex, scales = "free_y") + +
  # Add labels (title, subtitle, caption, x, y)
  labs(
      title = "Trends in the usage of vowels and consonants in names" ,
      subtitle = "Percentage of births by combination of first and last letter",
      caption = "Source:ssa",
      x = "Year",
      y = '% births'
    ) +
  ...
  )# unable to run this visualization
```

    #> Error in eval(expr, envir, enclos): object 'first_last' not found
